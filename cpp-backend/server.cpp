...
HttpResponse BackendServer::route(const HttpRequest& request) {
  ...
  if (request.path == "/api/code/file" && request.method == "GET") { const QString relative = QUrlQuery(request.url).queryItemValue("path"); const QString path = workspaceFile(relative); QFile file(path); if (path.isEmpty() || !file.open(QIODevice::ReadOnly)) return {400, errorResponse("文件不存在或不允许访问")}; return {200, QJsonObject{{"path", relative}, {"content", QString::fromUtf8(file.readAll())}}}; }
